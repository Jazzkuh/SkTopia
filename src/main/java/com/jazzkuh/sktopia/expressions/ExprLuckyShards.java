...
@Name("LuckyShards")
@Description({"The amount of luckyshards of a player"})
public class ExprLuckyShards extends SimpleExpression<Double> {
    private Expression<Player> player;

    public static void register() {
        Skript.registerExpression(ExprLuckyShards.class, Double.class, ExpressionType.COMBINED, "[the] [(minetopiasdb|sdb)] (luckyshards|lucky shards) of %player%");
    }
    ...
    protected Double[] get(final Event event) {
        ...
        if (sdbPlayer != null) {
            return new Double[] {sdbPlayer.getLuckyShards()};
        }
        return null;
    }
    ...
    public void change(Event event, Object[] delta, Changer.ChangeMode mode){
        ...
        if (sdbPlayer != null) {
            if (mode == Changer.ChangeMode.SET) {
                sdbPlayer.setLuckyShards((Double) delta[0]);
            } else if (mode == Changer.ChangeMode.ADD) {
                sdbPlayer.setLuckyShards(sdbPlayer.getLuckyShards() + (Double) delta[0]);
            } else if (mode == Changer.ChangeMode.REMOVE) {
                sdbPlayer.setLuckyShards(sdbPlayer.getLuckyShards() - (Double) delta[0]);
            } else if (mode == Changer.ChangeMode.DELETE || mode == Changer.ChangeMode.RESET) {
                sdbPlayer.setLuckyShards(0D);
            }
        }
    }
}
